...
def plot_history(epochs, Acc, Loss, lr, dropout_value):
    ...
    plt.savefig(f'history_Loss_dropout_{dropout_value}.png')
    # Save as a different file format (e.g., JPEG)
    plt.savefig(f'history_Loss_dropout_{dropout_value}.jpg', format='jpg')
    plt.show()
    ...
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig(f'history_Acc_dropout_{dropout_value}.png')
    # Save as a different file format (e.g., JPEG)
    plt.savefig(f'history_Acc_dropout_{dropout_value}.jpg', format='jpg')
    plt.show()

    plt.plot(epoch_list, lr)
    plt.xlabel('epoch')
    plt.ylabel('Train LR')
    plt.savefig(f'history_Lr_dropout_{dropout_value}.png')
    # Save as a different file format (e.g., JPEG)
...
if __name__ == '__main__':
    ...
    def train(net, trainloader, testloader, epochs, optimizer, criterion, scheduler, path='./model.pth', writer=None, verbose=False, dropout_value=0.5):
        ...
        for i in range(epochs):
            ...
            net.apply(lambda m: setattr(m, 'p', dropout_value) if isinstance(m, nn.Dropout) else None)
            net.train()
    ...
    save_path = './model/Dropout.pth'
    Acc, Loss, Lr = train(net, trainloader, testloader, epochs, optimizer, criterion, scheduler, save_path, verbose=True, dropout_value=args.dropout)
    plot_history(epochs, Acc, Loss, Lr, args.dropout)
